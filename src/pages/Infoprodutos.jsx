    ...
    <div style={{ paddingTop: '96px' }}>
      ...
      <section className="page-header" style={{ position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{
          ...
          background: 'radial-gradient(circle, rgba(232,161,78,0.14) 0%, transparent 70%)',
        ...
        <div className="container" style={{ position: 'relative', zIndex: 1, textAlign: 'center' }}>
          ...
          <div className="fade-up delay-3" style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            {['Acesso imediato', 'Aplicação prática', 'Para qualquer nicho'].map(t => (
              <span key={t} style={{
                background: 'rgba(51,103,202,0.08)',
                color: '#3367ca',
                borderRadius: 100,
                padding: '6px 16px',
                fontSize: 13,
            ...
            {products.map((p, i) => (
              <div
                ...
                style={{
                  ...
                  background: 'white',
                  borderRadius: 28,
                  boxShadow: '0 8px 40px rgba(62,58,83,0.10)',
                  overflow: 'hidden',
                  display: 'grid',
              ...
              >
                ...
                <div style={{ padding: '48px 48px 48px 48px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 16 }}>
                    ...
                    <div>
                      <h2 style={{ fontSize: 'clamp(22px, 2.5vw, 28px)', fontWeight: 800, color: 'var(--dark)', margin: 0 }}>
                        {p.title}
                      </h2>
                      <p style={{ fontSize: 13, color: 'var(--text-muted)', margin: 0, marginTop: 2 }}>{p.subtitle}</p>
                    </div>
                  </div>
                  ...
                  <div style={{
                    ...
                    {p.features.map(f => (
                      <div key={f.label} style={{
                        ...
                        borderRadius: 14,
                        padding: '14px 16px',
                        border: '1px solid rgba(62,58,83,0.06)',
                      }}>
                        <div style={{ fontSize: 18, marginBottom: 4 }}>{f.icon}</div>
                        <p style={{ fontWeight: 700, fontSize: 13, color: 'var(--dark)', margin: 0 }}>{f.label}</p>
                        <p style={{ fontSize: 12, color: 'var(--text-muted)', margin: 0, marginTop: 3, lineHeight: 1.5 }}>{f.desc}</p>
                      </div>
                    ))}
                  </div>
                      ...
                      <div key={h} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <div style={{
                          ...
                          <svg width="9" height="9" viewBox="0 0 12 12" fill="none">
                            <path d="M2 6l3 3 5-5" stroke={p.accentColor} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                          </svg>
                        </div>
                ...
                {/* Right: price card */}
                <div style={{
                  background: `linear-gradient(160deg, ${p.accentColor}08 0%, ${p.accentColor}04 100%)`,
                  borderLeft: `1px solid ${p.accentColor}18`,
                  padding: '48px 36px',
                  display: 'flex',
                  ...
                  <div style={{
                    ...
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    fontSize: 32,
                    border: `2px solid ${p.accentColor}20`,
                  }}>{p.emoji}</div>

                  <div>
                    <p style={{ fontSize: 13, color: 'var(--text-muted)', margin: 0, marginBottom: 4 }}>Investimento</p>
                    <p style={{ fontSize: 40, fontWeight: 900, color: p.accentColor, margin: 0, lineHeight: 1 }}>{p.price}</p>
                    <p style={{ fontSize: 12, color: 'var(--text-muted)', margin: 0, marginTop: 6 }}>{p.priceNote}</p>
                  </div>
                  ...
                  <a
                    ...
                    style={{
                      ...
                      fontSize: 15,
                      textDecoration: 'none',
                      boxShadow: `0 8px 24px ${p.accentColor}40`,
                      transition: 'all 0.2s',
                    }}
                  ...
                  <p style={{ fontSize: 12, color: 'var(--text-muted)', margin: 0 }}>
                    Você será redirecionada para<br />a página do produto
                  </p>
                </div>
              </div>
            ))}
        ...
        <div className="container">
          <div style={{
            ...
            <div className="fade-up" style={{ position: 'relative', zIndex: 1 }}>
              <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: 14, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 12 }}>
                Prefere uma solução personalizada?
              </p>
              <h2 style={{ color: 'white', fontSize: 'clamp(24px, 3vw, 36px)', fontWeight: 800, marginBottom: 16 }}>
